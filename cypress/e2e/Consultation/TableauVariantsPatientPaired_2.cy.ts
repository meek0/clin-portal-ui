...
describe('Page des variants d\'un patient (paired) - Consultation du tableau', () => { 
  ...
  it('Valider les liens disponibles Lien dbSNP', () => {
    cy.get('tr[data-row-key="8f11e237271fa6d6c5b0cab036012fe68fba0e7f"]').find('td').eq(6).find('a[href]')
      .should('have.attr', 'href', 'https://www.ncbi.nlm.nih.gov/snp/rs750458124');
  });
 
  it('Valider les liens disponibles Lien Gène', () => {
    cy.get('tr[data-row-key="2f53f2ed574a720853172ff224c608efc5e3b623"]').find('td').eq(7).find('a[href]')
      .should('have.attr', 'href', 'https://www.omim.org/entry/610467');
  });
 
  it('Valider les liens disponibles Lien Gène Plus', () => {
    cy.get('tr[data-row-key="2f53f2ed574a720853172ff224c608efc5e3b623"]').find('td').eq(7).find('[data-icon="plus"]').clickAndWait({force: true});
  ...
  it('Valider les liens disponibles Lien ClinVar', () => {
    cy.get('tr[data-row-key="2f53f2ed574a720853172ff224c608efc5e3b623"]').find('td').eq(11).find('a[href]')
      .should('have.attr', 'href', 'https://www.ncbi.nlm.nih.gov/clinvar/variation/1507888');
  });
 
  it('Valider les liens disponibles Lien Tier', () => {
    cy.get('tr[data-row-key="2f53f2ed574a720853172ff224c608efc5e3b623"]').find('td').eq(13).find('a[href]')
  ...
  it('Valider les liens disponibles Lien RQDM TN', () => {
    cy.get('tr[data-row-key="2f53f2ed574a720853172ff224c608efc5e3b623"]').find('td').eq(18).find('a[href]').clickAndWait({force: true});
    cy.validateTableResultsCount(/^1 Résultat$/);
  });
});
