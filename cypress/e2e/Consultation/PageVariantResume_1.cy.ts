...
describe('Page d\'un variant (onglet Résumé) - Vérifier les informations affichées', () => {
  ...
  it('Panneau ClinVar', () => {
    ...
    cy.get('[data-row-key*="ClinicalCard_ClinVar_3"] td[class="ant-table-cell"]').eq(1).contains('Cobalamin C disease').should('exist');
    ...
    cy.get('[data-row-key*="ClinicalCard_ClinVar_1"] td[class="ant-table-cell"]').eq(1).contains('not provided').should('exist');
    ...
    cy.get('[data-row-key*="ClinicalCard_ClinVar_0"] td[class="ant-table-cell"]').eq(1).contains('MMACHC-related disorder').should('exist');
