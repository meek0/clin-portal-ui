...
describe('Page d\'une prescription - Vérifier les informations affichées', () => {
  ...
  it('Panneau Analyse', () => {
    ...
    cy.get('[data-cy="AnalysisCard"] [class="ant-descriptions-item-content"]').eq(3).contains('Complétée').should('exist');
    ...
    cy.get('[data-cy="AnalysisCard"] [class="ant-descriptions-item-content"]').eq(7).contains(epCHUSJ_ldmCHUSJ.stampDate.substring(0, 4)).should('exist');
    cy.get('[data-cy="AnalysisCard"] [class="ant-descriptions-item-content"]').eq(8).contains(/(--|ALLARD René)/).should('exist');
    cy.get('[data-cy="AnalysisCard"] [class="ant-descriptions-item-content"]').eq(9).contains('CHUSJ').should('exist');
    cy.get('[data-cy="AnalysisCard"] [class="ant-descriptions-item-content"]').eq(10).contains('LDM-CHUSJ').should('exist');
  });
