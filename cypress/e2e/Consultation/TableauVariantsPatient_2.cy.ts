...
describe('Page des variants d\'un patient - Consultation du tableau', () => { 
  ...
  it('Valider les liens disponibles Lien Gène Plus', () => {
    cy.get('tr[data-row-key="4577893f4d3c2463e9fdef3419f7781d00fffdf3"] td').eq(7).find('[data-icon="plus"]').clickAndWait({force: true});
    cy.validatePillSelectedQuery('Gène', ['GRIA3']);
  });
 
  it('Valider les liens disponibles Lien OMIM', () => {
    cy.get('tr[data-row-key="4577893f4d3c2463e9fdef3419f7781d00fffdf3"] td').eq(10).find('a[href]')
      .should('have.attr', 'href', 'https://www.omim.org/entry/305915');
  });
 
  it('Valider les liens disponibles Lien ClinVar', () => {
    cy.get('tr[data-row-key="4577893f4d3c2463e9fdef3419f7781d00fffdf3"] td').eq(11).find('a[href]').eq(1)
  ...
  it('Valider les liens disponibles Lien ACMG F.', () => {
    ...
    cy.sortTableAndIntercept('ACMG F.', 1);
    cy.get('tr[class*="ant-table-row"]').eq(0).find('td').eq(14).find('a[href]').should('have.attr', 'href', 'https://franklin.genoox.com/clinical-db/variant/snp/chr1-248441241-C-T-HG38');
  });
 
  it('Valider les liens disponibles Lien RQDM G', () => {
    cy.get('tr[data-row-key="4577893f4d3c2463e9fdef3419f7781d00fffdf3"] td').eq(18).find('a[href]').clickAndWait({force: true});
  ...
  it('Valider les liens disponibles Lien Tier', () => {
    cy.get('tr[data-row-key="4577893f4d3c2463e9fdef3419f7781d00fffdf3"] td').eq(33).find('a[href]')
