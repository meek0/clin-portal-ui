...
const getParentTitle = (who: 'mother' | 'father', id: string, affected: boolean) => {
  ...
  return (
    <span className={cx(style.parentStatusTitle, 'parentStatusTitle')}>
      {`${intl.get(`screen.patientcnv.drawer.${who}.genotype`)} ${id ? `(${id})` : ''}`}
      {affected !== null ? (
        <Tooltip
          placement="right"
          title={
            affected
              ? intl.get('screen.patientcnv.drawer.affected')
              : intl.get('screen.patientcnv.drawer.notaffected')
          }
        >
    ...
    <Space size="large" direction="vertical">
      ...
      {(donor?.father_id || donor?.mother_id) && (
        ...
        >
          ...
          <Descriptions.Item label={intl.get('screen.patientcnv.drawer.transmission')}>
            {removeUnderscoreAndCapitalize(donor?.transmission! || '').defaultMessage(
              TABLE_EMPTY_PLACE_HOLDER,
            )}
          </Descriptions.Item>
          <Descriptions.Item label={intl.get('screen.patientcnv.drawer.parental.origin')}>
            {donor?.parental_origin ? capitalize(donor?.parental_origin) : TABLE_EMPTY_PLACE_HOLDER}
          </Descriptions.Item>
        </Descriptions>
      )}
      <Descriptions
        title={intl.get('screen.patientcnv.drawer.seq.method')}
        column={1}
        className={cx(style.description, 'description')}
      >
        <Descriptions.Item label={intl.get('screen.patientcnv.drawer.depth.quality')}>
          {donor?.qd ?? TABLE_EMPTY_PLACE_HOLDER}
        </Descriptions.Item>
        <Descriptions.Item label={intl.get('screen.patientcnv.drawer.allprof')}>
          {donor?.ad_alt ?? TABLE_EMPTY_PLACE_HOLDER}
        </Descriptions.Item>
        <Descriptions.Item label={intl.get('screen.patientcnv.drawer.alltotal')}>
          {donor?.ad_total ?? TABLE_EMPTY_PLACE_HOLDER}
        </Descriptions.Item>
        <Descriptions.Item label={intl.get('screen.patientcnv.drawer.allratio')}>
          {donor?.ad_ratio ? donor?.ad_ratio.toFixed(2) : TABLE_EMPTY_PLACE_HOLDER}
        </Descriptions.Item>
        <Descriptions.Item label={intl.get('screen.patientcnv.drawer.gq')}>
          {donor?.gq ? donor.gq : TABLE_EMPTY_PLACE_HOLDER}
        </Descriptions.Item>
        <Descriptions.Item label={intl.get('screen.patientcnv.drawer.filter')}>
          {donor?.filters}
        </Descriptions.Item>
      </Descriptions>
      ...
      <Space>
        ...
        <ReportButton
          ...
          tooltipTitle={intl.get('screen.patientcnv.drawer.download.report.tooltip')}
          size={'middle'}
        />
      </Space>
    </Space>
